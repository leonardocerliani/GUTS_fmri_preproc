...
preproc_data_dir <- "./preproc_data_MNI"
...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(6,
      tags$h4("TSNR - range 0..300"),
      papayaOutput('papayaView2', height = "600px"),
      tags$h4("FD Mean vs GM TSNR - Marker size = %TR with FD > 0.4"),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (!is.null(click_data)) {
      clicked_sub <- click_data$customdata
      if (!is.null(clicked_sub) && length(clicked_sub) == 1) {
        updateSelectInput(session, "subject", selected = clicked_sub)
      }
    }
  })
  ...
  output$scatterPlot <- renderPlotly({
    plot_ly(
      ...
      textfont = list(color = "gray", family = "Arial Narrow"),
      hoverinfo = 'text',
      hovertext = ~paste0(
        "Subject: ", sub_id, "<br>",
      ...
      customdata = ~sub_id    # <--- add this here
    ) %>%
      layout(
